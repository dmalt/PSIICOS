function [corr, Cpvec, Upwr] = PSIICOS_svd_re_im_split(C, G2dU, SL_rnk,...
                                       sig_rnk, Upwr,...
                                       seed_ind, cp_part,...
                                       is_fast)
% ------------------------------------------------------------
...
% Alex Ossadtchii ossadtchi@gmail.com, Dmitrii Altukhov, dm.altukhov@ya.ru
    ...
    if sig_rnk
        ...
        elseif strcmp(cp_part, 'full')
            [uc_re,~,~] = svd(real(Cpvec), 'econ');
            [uc_im,~,~] = svd(imag(Cpvec), 'econ');
            Cp = [uc_re(:, 1:sig_rnk), uc_im(:,1:sig_rnk)];
            size(Cp)
        end
